...
from Happy_4.output2 import output_loop, checkout
...
#check if we can get the total power dataframe
    t,j = solar_input.solar(33.2164, -97.1292,2010)
    m,n = wind_input.wind(33.2164, -97.1292,2010)
    d= output_loop(2010,0.5,0.5,t,m)
...
#check if we can get right total value from wind and solar dataframe
    i=0
    assert output_loop(2010,0.5,0.5,t,m).iloc[i]['generation']==0.5*x.iloc[i]['generation'] + 0.5*m.iloc[i]['generation'],'cannot get right total energy value'
...
def test_checkout():
    ...
    for x in np.arange(0.1,1,0.001):
        ...
        a.append(x)
        b.append(y)
        c.append(output_loop(2010,x,y,t,m).std().values[0])
